...
def forward_pass(input_embedding, fn_model, model, target_sequence):
    ...
    # calculate the log-likelihood of the target-sequence for this output
    d_likelihood = llikelihood(outputs, target_sequence.squeeze())
    
    return d_likelihood

def training_loop(input_embedding, fn_forward_pass, optimizer, n=1000):
    ...
    for i in tqdm(range(n)):
        ...
        before_step = input_embedding.detach().numpy()
        
        d_likelihood = fn_forward_pass(input_embedding)
        print(d_likelihood)
        d_likelihood.backward()
        
        optimizer.step()
        
        losses.append(d_likelihood)

        # collect parameters again
        after_step = input_embedding.detach().numpy()

        # Print if parameters are the same or not
        if np.array_equal(before_step, after_step):
            print("No change in parameter")
        else:
            print("CHANGE")
...
fn_forward_pass = lambda x: forward_pass(x, fn_model, model, target_sequence)
...
# CITE: ecco package: https://github.com/jalammar/ecco/blob/main/src/ecco/attribution.py
# TODO: extra_forward_args=extra_forward_args
def model_forward(input_: torch.Tensor, model) -> torch.Tensor:
        output = model(inputs_embeds=input_)
